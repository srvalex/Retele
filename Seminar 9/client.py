...
while True:
    ...
    if comanda == 'EXIT':
        ...
        elif len(parti) < 2 or not parti[1].strip():
            print("EROARE: Trebuie sa furnizezi un mesaj. Exemplu: PUBLISH textul meu")
        else:
            raspuns = trimite_comanda(intrare)
            print(raspuns)
        ...
        if not este_conectat:
            print("EROARE: Nu esti conectat. Foloseste CONNECT mai intai.")
        elif len(parti) < 2 or not parti[1].strip().isdigit():
            print("EROARE: Trebuie sa furnizezi un ID numeric. Exemplu: DELETE 3")
        else:
            raspuns = trimite_comanda(intrare)
            print(raspuns)
